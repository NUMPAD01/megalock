    ...
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-card/95 backdrop-blur-sm border-b border-card-border h-14 flex items-center px-4">
        ...
        <Link href="/" className="ml-3 flex items-center gap-2">
          <img src="/logo.jpg" alt="TempoScan" className="w-6 h-6 rounded-full" />
          <span className="text-lg font-bold text-primary">TempoScan</span>
        </Link>
      </div>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 bottom-0 z-50 w-60 bg-[#0d0d0d] border-r border-card-border flex flex-col transition-transform duration-200 ease-in-out
          ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}
      >
        {/* Brand */}
        <div className="flex items-center justify-between h-14 px-5 border-b border-card-border shrink-0 accent-line">
          <Link href="/" className="flex items-center gap-2.5" onClick={() => setSidebarOpen(false)}>
            <img src="/logo.jpg" alt="TempoScan" className="w-7 h-7 rounded-full" />
            <span className="text-xl font-bold gradient-text">TempoScan</span>
          </Link>
          <button
            onClick={() => setSidebarOpen(false)}
        ...
        <div className="shrink-0 border-t border-card-border p-4 space-y-4">
          ...
          {isConnected && address && (
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-xs font-medium truncate">My Wallet</p>
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            {/* Enshrined Exchange */}
            <a
              href="https://launch.enshrined.exchange/token/0x20c00000000000000000000088f2ce96f78Fa037"
              target="_blank"
              rel="noopener noreferrer"
              title="Enshrined Exchange"
              className="flex items-center gap-1.5 text-muted hover:text-foreground transition-colors"
            >
              <img src="/enshrined.jpg" alt="Enshrined" className="w-4 h-4 rounded-sm" />
              <span className="text-xs font-semibold">Enshrined</span>
            </a>
          </div>
        </div>
      </aside>
