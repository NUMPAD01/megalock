...
import { TEMPOLOCK_ADDRESS, TEMPOLOCK_ABI, ERC20_ABI } from "@/lib/contracts";
...
function TimelockForm() {
  ...
  const handleApprove = () => {
    ...
    approve({
      ...
      args: [TEMPOLOCK_ADDRESS, parseUnits(amount, decimals)],
    });
  };
  ...
  const handleCreate = () => {
    ...
    createLock({
      address: TEMPOLOCK_ADDRESS, abi: TEMPOLOCK_ABI, functionName: "createTimeLock",
      args: [
  ...
  return (
    <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
      ...
      {step === "approve" && !isApproveConfirmed ? (
        <button onClick={handleApprove} disabled={isApproving || isApproveConfirming || !token || !amount || isAmountInvalid} className="w-full bg-primary hover:bg-primary-hover disabled:opacity-50 text-black font-medium py-3 px-4 rounded-lg transition-colors">
          {isApproving ? "Sign in wallet..." : isApproveConfirming ? "Confirming..." : "Approve Token"}
        </button>
      ) : (
        <button onClick={handleCreate} disabled={isCreating || isCreateConfirming || !token || !beneficiary || !amount || !unlockDate || isAmountInvalid} className="w-full bg-primary hover:bg-primary-hover disabled:opacity-50 text-black font-medium py-3 px-4 rounded-lg transition-colors">
...
function LinearVestingForm() {
  ...
  const handleApprove = () => {
    if (!token || !amount || amountNum <= 0) return;
    approve({ address: token as `0x${string}`, abi: ERC20_ABI, functionName: "approve", args: [TEMPOLOCK_ADDRESS, parseUnits(amount, decimals)] });
  };

  const handleCreate = () => {
    if (!token || !beneficiary || !amount || !startDate || !endDate || amountNum <= 0) return;
    createLock({
      address: TEMPOLOCK_ADDRESS, abi: TEMPOLOCK_ABI, functionName: "createLinearVesting",
      args: [
        token as `0x${string}`, beneficiary as `0x${string}`, parseUnits(amount, decimals),
  ...
  return (
    <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
      ...
      {step === "approve" && !isApproveConfirmed ? (
        ...
        <button onClick={handleCreate} disabled={isCreating || isCreateConfirming || !token || !beneficiary || !amount || !startDate || !endDate || isAmountInvalid} className="w-full bg-primary hover:bg-primary-hover disabled:opacity-50 text-black font-medium py-3 px-4 rounded-lg transition-colors">
...
function SteppedVestingForm() {
  ...
  const handleCreate = () => {
    ...
    createLock({
      address: TEMPOLOCK_ADDRESS, abi: TEMPOLOCK_ABI, functionName: "createSteppedVesting",
  ...
  return (
    <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
      ...
      {step === "approve" && !isApproveConfirmed ? (
        ...
        <button onClick={handleCreate} disabled={isCreating || isCreateConfirming || !token || !beneficiary || !amount || totalPercentage !== 100 || isAmountInvalid} className="w-full bg-primary hover:bg-primary-hover disabled:opacity-50 text-black font-medium py-3 px-4 rounded-lg transition-colors">
