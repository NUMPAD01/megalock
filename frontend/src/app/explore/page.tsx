...
import { TEMPOLOCK_ADDRESS, TEMPOLOCK_ABI, TEMPOBURN_ADDRESS, TEMPOBURN_ABI, ERC20_ABI } from "@/lib/contracts";
...
export default function ExplorePage() {
  ...
  // Fetch all burn events from TempoBurn contract
  useEffect(() => {
    ...
    <div className="space-y-6">
      <FadeIn>
        <div>
          ...
          <p className="text-muted mt-2">Browse all locks and burns on TempoScan</p>
        </div>
      </FadeIn>
      ...
      <div className="p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right flex items-center gap-2">
            <span className="text-sm font-semibold">{formatTokenAmount(lock.totalAmount, dec)} {tokenSymbol || "tokens"}</span>
        ...
        <div className="border-t border-card-border p-4 space-y-4" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-background rounded-lg p-3">
              ...
              <p className="font-semibold text-sm">{formatTokenAmount(lock.totalAmount, dec)} {tokenSymbol}</p>
            </div>
            <div className="bg-background rounded-lg p-3">
              <p className="text-muted text-[10px]">Vested</p>
              <p className="font-semibold text-sm text-primary">{vested ? formatTokenAmount(vested, dec) : "0"} {tokenSymbol}</p>
              <p className="text-[10px] text-muted">{vestedPercent.toFixed(1)}%</p>
            </div>
            <div className="bg-background rounded-lg p-3">
              <p className="text-muted text-[10px]">Claimed</p>
              <p className="font-semibold text-sm">{formatTokenAmount(lock.claimedAmount, dec)} {tokenSymbol}</p>
            </div>
            <div className="bg-background rounded-lg p-3">
              <p className="text-muted text-[10px]">Remaining</p>
              <p className="font-semibold text-sm">{formatTokenAmount(remaining, dec)} {tokenSymbol}</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex gap-3">
              <a href={`https://explore.mainnet.tempo.xyz/address/${TEMPOLOCK_ADDRESS}`}
                target="_blank" rel="noopener noreferrer"
                className="text-xs text-primary hover:underline"
                onClick={(e) => e.stopPropagation()}>View on Explorer</a>
