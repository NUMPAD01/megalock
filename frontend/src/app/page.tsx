...
export default function Dashboard() {
  ...
  // No explorer API on Tempo - watchlist market data not available
  ...
  // No explorer search API on Tempo - address-only search
  ...
  useEffect(() => {
    ...
    const fetchActivity = async () => {
      ...
      try {
        // Get current block and scan last ~100k blocks for recent activity
        const currentBlock = await publicClient.getBlockNumber();
        const fromBlock = currentBlock > 100000n ? currentBlock - 100000n : 0n;

        const [lockLogs, burnLogs] = await Promise.all([
          publicClient.getLogs({
            address: TEMPOLOCK_ADDRESS,
            event: parseAbiItem(
            ...
            {watchlist.map((token, i) => (
              <FadeIn key={token.address} delay={i * 50}>
                <div className="bg-card border border-card-border rounded-xl p-4 card-glow card-texture group relative">
                  ...
                  <a href="https://launch.enshrined.exchange/" target="_blank" rel="noopener noreferrer"
            ...
            <Link href="/burn" className="bg-card border border-card-border rounded-xl p-5 card-glow card-texture transition-all group block">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-muted text-sm mt-0.5">Permanently destroy tokens via TempoBurn</p>
                </div>
              </div>
            </Link>
          </FadeIn>
        ...
        <FadeIn>
          <div className="border-t border-card-border pt-8">
            ...
            <p className="text-muted text-sm max-w-xl leading-relaxed">
              TempoScan is a token toolkit on Tempo. Lock liquidity, create vesting schedules for your team,
