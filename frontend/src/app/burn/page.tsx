...
import { TEMPOBURN_ADDRESS, TEMPOBURN_ABI, ERC20_ABI } from "@/lib/contracts";
...
export default function BurnPage() {
  ...
  const { data: userBurnedAmount } = useReadContract({
    address: TEMPOBURN_ADDRESS, abi: TEMPOBURN_ABI, functionName: "userBurned",
  ...
  const handleApprove = () => {
    ...
    approve({ address: token as `0x${string}`, abi: ERC20_ABI, functionName: "approve", args: [TEMPOBURN_ADDRESS, parseUnits(amount, decimals)] });
  };

  const handleBurn = () => {
    if (!token || !amount || amountNum <= 0) return;
    burn({ address: TEMPOBURN_ADDRESS, abi: TEMPOBURN_ABI, functionName: "burn", args: [token as `0x${string}`, parseUnits(amount, decimals)] });
  };
